...
__global__ void ComputeLamda(float* g_VecV, float* g_VecW, float* g_Lamda, int N) {
    int threads = blockIdx.x * blockDim.x + threadIdx.x;
    float result_per_threads = 0.0f;
    if (threads < N) {
        result_per_threads = g_VecV[threads] * g_VecW[threads];

        atomicAdd(g_Lamda, result_per_threads);
...
__global__ void FindNormW(float* g_VecW, float* g_NormW, int N) {
    ...
    if (threads < N) {
        ...
        result_per_threads = g_VecW[threads] * g_VecW[threads];
        atomicAdd(g_NormW, result_per_threads);
        //printf("g_NormW: %f\n", g_NormW[0]);
    }
...
__global__ void NormalizeW(float* g_VecW, float* g_NormW, float* g_VecV, int N) {
    int threads = blockIdx.x * blockDim.x + threadIdx.x;
    if (threads < N) {
        g_VecV[threads] = g_VecW[threads] / g_NormW[0];
    }
}


__global__ void Av_Product(float* g_MatA, float* g_VecV, float* g_VecW, int N) {
    int row_thread = blockIdx.y * blockDim.y + threadIdx.y;
    int column_thread = blockIdx.x * blockDim.x + threadIdx.x;
    if (column_thread < N) {

        float result_per_thread = 0.0f;
        float* inter_result = &result_per_thread;
        for (int i = 0; i < N; i++) {

            result_per_thread+=g_MatA[column_thread*N+i] * g_VecV[i];
            //printf("inter_result:%f\n", g_VecV[column_thread]);
        }
        g_VecW[column_thread] = result_per_thread;
    }
}
