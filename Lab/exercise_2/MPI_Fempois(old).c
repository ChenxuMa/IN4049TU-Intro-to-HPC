...
//               proc_rank, wtime, 100.0 * ticks * (1.0 / CLOCKS_PER_SEC) / wtime);
        printf("(%i) Elapsed Wtime: %14.6f s\n",
               proc_rank,timer_solve_total);
        printf("(%i) exchange border time:%.6f\ncomputation time:%.6f\nglobal communication time:%.6f\ntotal idle time:%.6f\n", proc_rank, timer_exchange_borders_total, timer_computation_total, timer_global_comm_total, timer_idle);
        printf("total communication time:%.6f\n", timer_global_comm_total+timer_exchange_borders_total);
        resume_timer();
    }
    else
    {
